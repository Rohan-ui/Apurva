  ...
  const sidebarData = [
    ...
    },
    {
      title: "About us", icon: <BsFillPersonLinesFill size={20} />, submenu: [
        ...
        { title: "Packaging", icon: <GoGoal size={20} />, path: "/packagingdetail" },
        { title: "Packaging Type", icon: <GoGoal size={20} />, path: "/packagingType" },
        // { title: "Our Team", icon: <FaPeopleGroup size={20} />, path: "/ourTeam" },
        { title: "Our Partners", icon: <FaPeopleGroup size={20} />, path: "/partners" },
        { title: "Our People", icon: <FaPeopleGroup size={20} />, path: "/ourpeople" },
      ]
    },
  ...
  return (
    <div className='flex w-full'>
      <div className='h-screen'>
        <aside
          ref={sidebarRef}
          className={` bg-gray-400 text-black fixed lg:relative z-10 h-full w-[14rem] md:w-[18rem] overflow-y-auto  ${isMenuOpen ? "translate-x-0 transform transition-transform duration-500" : "-translate-x-full lg:translate-x-0"
            }`}>
          <div className='font-bold text-black text-center pt-4 text-[20px] px-8'>
            {logo && logo.length > 0 && (
              <div>
                <img src={`/api/logo/download/${logo[logo.length - 1].photo}`} alt="Logo" className="w-[4cm] " />
              </div>
            )}
          </div>
          <div className='mt-4'>
              ...
              {sidebarData.map((item, i) => (
                <div key={i}>
                  <div>
                    <Link
                      to={item.path || "#"}
                      className={`flex items-center gap-2 hover:bg-slate-700 py-2 pl-4 pr-8 hover:cursor-pointer ${location.pathname === item.path ? "bg-slate-700" : ""}`}
                      ...
                      <ul>
                        {item.submenu.map((subItem, j) => (
                          <div key={j}>
                            <Link
                              to={subItem.path || "#"}
                              className={`flex items-center gap-2 hover:bg-slate-700 py-2 pl-8 pr-4 hover:cursor-pointer ${location.pathname === subItem.path ? "bg-slate-700" : ""}`}
                              onClick={subItem.subsubmenu && subItem.subsubmenu.length > 0 ? (e) => toggleSubSubMenu(e, i, j) : undefined}
                            >
                              <p className='text-black'>{subItem.icon}</p>
                              <p className='text-black font-semibold'>{subItem.title}</p>
                              ...
                              <ul>
                                {subItem.subsubmenu.map((subSubItem, k) => (
                                  <Link
                                    key={k}
                                    to={subSubItem.path}
                                    className={`flex items-center gap-2 hover:bg-slate-700 py-2 pl-12 pr-16 hover:cursor-pointer ${location.pathname === subSubItem.path ? "bg-slate-700" : ""}`}
                                  >
                                    <p className='text-black'>{subSubItem.title}</p>
                                  </Link>
                                ))}
                              </ul>
                            }
      ...
      <div className="flex flex-col h-screen w-full">
        <Navbar className="fixed w-full z-10 bg-black shadow" toggleSidebar={toggleSidebar} />
