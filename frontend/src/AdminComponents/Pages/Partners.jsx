...
const PartnersTable = () => {
        ...
        Cell: ({ row }) => (
          <div className="flex gap-4">
            <button className="text-blue-500 hover:text-blue-700 transition">
              <Link to={`/partners/editPartners/${row.original._id}`}>  <FaEdit /></Link>
            </button>
  ...
  const fetchData = async (pageIndex) => {
    setLoading(true);
    try {
      const response = await axios.get(`/api/partners/getPartners?page=${pageIndex + 1}`, { withCredentials: true });
    ...
    <div className="p-4 overflow-x-auto">
      ...
      <div className="flex justify-between items-center mb-4">
        ...
        <button className="px-4 py-2 bg-slate-700 text-white rounded hover:bg-slate-900 transition duration-300">
          <Link to="/partners/createPartners"><FaPlus size={15} /></Link>
        </button>
      </div>
      <div className="mb-4">
        <input
