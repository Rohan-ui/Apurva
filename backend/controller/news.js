...
const insertNews = async (req, res) => {
  try {
    ...
    const { title, details, status, alt, imgTitle, slug, productId, metatitle, metadescription, metakeywords, metacanonical, metalanguage, metaschema, otherMeta, postedBy, date, categories, url, priority, changeFreq } = req.body;
  ...
  try {
    ...
    const newsWithCategoryNameAndProductName = await Promise.all(news.map(async (newsItem) => {
      ...
      const productNames = await Promise.all(
        news.map(async (productDetail) => {
          const productNameRecord = await Product.findOne({ 'slug': productDetail.productId });
          return productNameRecord ? productNameRecord.title : 'Unnamed';
        })
      );
...
const getActiveNews = async (req, res) => {
  try {
    ...
    const activeNewsWithCategoryNameAndProductName = await Promise.all(activeNews.map(async (newsItem) => {
      // Fetch category name
      const category = await newsCategory.findOne({ '_id': newsItem.categories });
      const categoryName = category ? category.category : 'Uncategorized';
      ...
      // Fetch product names if there are associated products
      const productNames = await Promise.all(
        activeNews.map(async (productDetail) => {
          const productNameRecord = await Product.findOne({ 'slug': productDetail.productId });
          return productNameRecord ? productNameRecord.title : 'Unnamed';
        })
      );
      ...
      return {
        ...newsItem.toJSON(),
        categoryName,
        productNames // Add the product names to the response
      };
    }));

    // Send response with the active news data
    res.status(200).json({
      data: activeNewsWithCategoryNameAndProductName,
      total: activeNewsWithCategoryNameAndProductName.length
    });
  } catch (error) {
...
const getLatestActiveNews = async (req, res) => {
  ...
  module.exports = { getDataBySlug, getBlogsByCategory, getLatestActiveNews, getActiveNews, insertNews, getNews, updateNews, deleteNews, getNewsById, countNews, deletePhotoAndAltText, getCategoryNews, fetchUrlPriorityFreq, editUrlPriorityFreq, fetchUrlPriorityFreqById, fetchUrlmeta, editUrlmeta, fetchUrlmetaById };
