...
const deleteBanner = async (req, res) => {
  ...
  const getBannersBySectionContactus = async (req, res) => {
    try {
      const banners = await Banner.find({ section: 'Contact',status: 'active' }).sort({ priority: 1 });;
      res.status(200).json({
        data: banners,
  ...
  const getBannersBySectionCustomPackaging = async (req, res) => {
    try {
      const banners = await Banner.find({ section: 'Custom Packaging',status: 'active' }).sort({ priority: 1 });;
      res.status(200).json({
        data: banners,
        message: "Banners for Products section fetched successfully",
      });
...
module.exports = {getBannersBySectionCustomPackaging,getBannersBySectionBlog,getBannersBySectionContactus,getBannersBySectionProducts, getBannersBySectionAboutus,insertBanner, getBanner ,getBannersBySectionHome,getCountBySection,updateBanner , deleteBanner , getBannerById,deletePhotoAndAltText,countBanner };
