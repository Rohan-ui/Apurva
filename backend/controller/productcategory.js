...
const insertCategory = async (req, res) => {
  const { category,description, alt, imgTitle, slug, metatitle, metadescription, metakeywords, metacanonical, metalanguage, metaschema, otherMeta, url, priority, changeFreq } = req.body;
  ...
  try {
    ...
    const newCategory = new ProductCategory({ category, description,alt, imgTitle, photo, slug, metatitle, metadescription, metakeywords, metacanonical, metalanguage, metaschema, otherMeta, url, priority, changeFreq });
...
//     categoryDoc.subCategories.push({ category, description,alt, imgTitle, photo, slug, metatitle, metadescription, metakeywords, metacanonical, metalanguage, metaschema, otherMeta, url, priority, changeFreq });
...
// const getAllSubCategoryDetails = async (req, res) => {
//   try {
//     // Find all category documents
...
//     // Extract only the desired fields from each subcategory
...
//   try {
//     // Extract the category slug from query parameters
...
//     // Find the category document based on the slug
//     const categoryDoc = await ProductCategory.findOne({ 'slug': category });
...
//       photo: categoryDoc.photo,
//       alt: categoryDoc.alt,
//       imgTitle: categoryDoc.imgTitle,
//       slug:categoryDoc.slug
//     };

//     // Extract all subcategories from the found category document
//     const subCategoryDetails = categoryDoc.subCategories.map(subCat => ({
//       category: subCat.category,
//       description: subCat.description,
//       photo: subCat.photo,
//       slug: subCat.slug
...
//     // Respond with the category details and subcategories
//     res.status(200).json({ category: categoryDetails, subCategories: subCategoryDetails });
//   } catch (error) {
//     res.status(500).json({ message: 'Server error', error });
//   }
// };
...
//   try {
//     // Extract the categorySlug and subCategorySlug from query parameters
//     const { categorySlug, subCategorySlug } = req.query;

//     if (!categorySlug || !subCategorySlug) {
//       return res.status(400).json({ message: 'Category slug and subcategory slug query parameters are required' });
//     }

//     // Find the category document based on the categorySlug
//     const categoryDoc = await ProductCategory.findOne({ slug: categorySlug });

//     if (!categoryDoc) {
//       return res.status(404).json({ message: 'Category not found' });
//     }

//     // Find the subcategory within the found category document
//     const subCategory = categoryDoc.subCategories.find(subCat => subCat.slug === subCategorySlug);
...
// const getCategoryAndSubCategoryDetails = async (req, res) => {
//   try {
//     // Find all category documents
//     const categories = await ProductCategory.find();
...
//   } catch (error) {
//     console.error('Error retrieving category and subcategory details:', error);
//     res.status(500).json({ message: 'Server error', error });
//   }
// };
...
const updateCategory = async (req, res) => {
  const { categoryId } = req.query; // Old slug from query params
  ...
  if (req.file) {
    photo = req.file.filename; // Update the photo if a new file is uploaded
  }

  try {
    // Update the ProductCategory with the new data
    const updatedCategory = await ProductCategory.findOneAndUpdate(
      { slug: categoryId }, // Find category by old slug
      {
        category,
        description,
        alt,
        imgTitle,
        photo,
        slug, // Update slug with new value
        metatitle,
        metadescription,
        metakeywords,
        metacanonical,
    ...
    // Check if old slug exists as categoryId in Product collection
    const updatedProducts = await Product.updateMany(
      { categories: categoryId }, // Find products where old slug exists as categoryId
      { categories: slug }, // Replace old slug with new slug
    ...
    if (updatedProducts.nModified > 0) {
      res.status(200).json({
        updatedCategory,
        message: `${updatedProducts.nModified} products updated with new category slug`,
      });
    } else {
      res.status(200).json({
        updatedCategory,
        message: 'No products were associated with the old category slug',
      });
    }
  } catch (error) {
    res.status(500).json({ message: 'Server error', error });
...
//   try {
//     const categoryDoc = await ProductCategory.findOne({slug:categoryId});
//     if (!categoryDoc) {
//       return res.status(404).json({ message: 'Category not found' });
//     }
...
//     const subCategoryIndex = categoryDoc.subCategories.findIndex(subCat => subCat.slug.toString() === subCategoryId);
...
const fetchCategoryUrlPriorityFreq = async (req, res) => {
  try {
    const categories = await ProductCategory.find({}, 'id url changeFreq priority lastmod ')
      // .populate({
      //   path: 'subCategories',
      //   select: 'id url changeFreq priority lastmod',
        // populate: {
        //   path: 'subSubCategory',
...
const getCategoryAndPhoto = async (req, res) => {
  try {
    const categories = await ProductCategory.find().select('category slug');
